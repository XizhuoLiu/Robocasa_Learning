...
from robomimic.utils.bspline_utils import batch_bspline
...
class TransformerGMMBSplineActorNetwork(TransformerGMMActorNetwork):
    ...
    def _get_output_shapes(self):
        return OrderedDict(
            mean=(self.num_modes, self.num_control_points, self.ac_dim),
            scale=(self.num_modes, self.num_control_points, self.ac_dim),
            logits=(self.num_modes,)
        )
    ...
    def forward_train(self, obs_dict, actions=None, goal_dict=None, low_noise_eval=None):
        ...
        component_distribution = D.Normal(loc=means, scale=scales)
        component_distribution = D.Independent(component_distribution, 2)
        mixture_distribution = D.Categorical(logits=logits)
    ...
    def forward(self, obs_dict, actions=None, goal_dict=None):
        ...
        full_cp = torch.cat([
            ...
            ctrl_pts,
            goal_pos.unsqueeze(1),
        ], dim=1)

        t_values = torch.linspace(0, 1, steps=self.context_length + 1, device=full_cp.device)

        pred_traj = batch_bspline(full_cp, t_values=t_values)
        pred_actions = pred_traj[:, 1:] - pred_traj[:, :-1]

        return pred_actions

    def _to_string(self):
        return (
            f"control_point_dim={self.ac_dim}, "
            f"num_control_points={self.num_control_points}, "
            f"std_activation={self.std_activation}, "
            f"low_noise_eval={self.low_noise_eval}, "
            f"num_modes={self.num_modes}, "
            f"min_std={self.min_std}"
        )   
    
    def sample(self, obs_dict, goal_dict=None):
        return self.forward(obs_dict, goal_dict=goal_dict)
