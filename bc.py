...
def algo_config_to_class(algo_config):
    ...
    bspline_enabled = ("bspline" in algo_config and algo_config.bspline.enabled)
    ...
    elif gmm_enabled and bspline_enabled and transformer_enabled:
        algo_class, algo_kwargs = BC_Transformer_GMM_BSpline, {}
    elif gmm_enabled:
        if rnn_enabled:
...
class BC_Transformer_GMM_BSpline(BC_Transformer_GMM):

    def _create_networks(self):
        
        assert self.algo_config.gmm.enabled
        assert self.algo_config.transformer.enabled
        assert self.algo_config.bspline.enabled

        self.nets = nn.ModuleDict()
        self.nets["policy"] = PolicyNets.TransformerGMMBSplineActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim,
            num_modes=self.algo_config.gmm.num_modes,
